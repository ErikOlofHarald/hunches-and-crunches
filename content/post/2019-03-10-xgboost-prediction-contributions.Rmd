...
I'm working with the same insurance frequency model using four numerical features as input. The label is the number of claims on the policy during the exposure period, and the logarithm of this exposure period, expressed as a fraction of a year, is set as an offset.
...
Denoting the sum of the first and second derivates of the loss function in node $j$ as $G_j$ and $H_j$ respectively, the weight can be calculated as
...
This is the weighted average of the importances attributed to including `NCD` with and without `VAgeCat` in the model. To get the SHAP values from the first tree of our example observation the same calculation has to be done for $\phi_{VAgeCat}$
...
These calculations have to be done for all trees and aggregated to get contributions from the entire ensamble. In our example, the second tree has the same structure as the first.
